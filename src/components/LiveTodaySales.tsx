...
interface Data { date: string; total_qtd: number; total_valor: number; by_product: ByProduct[]; }
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-br from-[#1a1a1a] to-[#0d0d0d] border border-[#2a2a2a] p-4 sm:p-5">
      <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-[#666]">produto e quantidade vendida</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="text-right">
            <div className="text-[9px] text-[#888] uppercase tracking-wider">Total</div>
            <div className="text-sm font-black text-emerald-400 tabular-nums leading-none">{data?.total_qtd ?? 0} · {brl(data?.total_valor ?? 0)}</div>
          </div>
      ...
      {loading && !data ? (
        ...
        <p className="text-[12px] text-[#666] py-8 text-center">Sem vendas nesse dia.</p>
      ) : (
        <div className="space-y-1.5">
          ...
          {byProduct.map((p, i) => (
            <div key={i} className="relative rounded-lg bg-[#111] border border-[#222] overflow-hidden">
              ...
              <div className="absolute inset-y-0 left-0 bg-emerald-500/10" style={{ width: `${(p.qty / maxQty) * 100}%` }} />
              <div className="relative flex items-center gap-2 px-2.5 py-2.5">
                ...
                <span className="w-24 text-right text-[12px] font-bold text-emerald-400 tabular-nums">{brl(p.revenue)}</span>
              </div>
            </div>
          ))}
        </div>
