...
-- Update get_dashboard_summary_today to include variant E
...
BEGIN
  ...
  ab_sales_data AS (
    ...
    INNER JOIN ab_today_sessions ats ON ats.session_id = p.effective_session_id
...
-- Update get_ab_summary_by_date to include variant E
...
BEGIN
  ...
  ab_sales_data AS (
    SELECT ads.variant,
      ...
      COALESCE(SUM(p.amount), 0) AS total_revenue
    FROM enriched_purchases p
    INNER JOIN ab_date_sessions ads ON ads.session_id = p.effective_session_id
    WHERE p.status = 'approved'
    GROUP BY 1
  ),
  unattributed AS (
    SELECT
      ...
      COALESCE((SELECT COUNT(DISTINCT p.id) FROM enriched_purchases p
        LEFT JOIN ab_date_sessions ads ON ads.session_id = p.effective_session_id
        WHERE p.status = 'approved' AND p.funnel_step LIKE 'front%'
          AND ads.session_id IS NULL), 0) AS orphan_sales
...
-- Update get_ab_summary_range to include variant E
...
BEGIN
  ...
  ab_sales_data AS (
    SELECT ars.variant,
      ...
      COALESCE(SUM(p.amount), 0) AS total_revenue
    FROM enriched_purchases p
    INNER JOIN ab_range_sessions ars ON ars.session_id = p.effective_session_id
    WHERE p.status = 'approved'
    GROUP BY 1
  ),
  ab_combined AS (
