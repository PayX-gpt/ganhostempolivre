...
-- Helper function to decode URL-encoded strings in PostgreSQL
...
-- Fix Divergence 7: Retroactively decode URL-encoded UTMs
UPDATE public.session_attribution SET utm_campaign = public.url_decode(utm_campaign) WHERE utm_campaign LIKE '%\%%';
UPDATE public.session_attribution SET utm_content = public.url_decode(utm_content) WHERE utm_content LIKE '%\%%';
UPDATE public.session_attribution SET utm_source = public.url_decode(utm_source) WHERE utm_source LIKE '%\%%';
UPDATE public.session_attribution SET utm_medium = public.url_decode(utm_medium) WHERE utm_medium LIKE '%\%%';
UPDATE public.session_attribution SET utm_term = public.url_decode(utm_term) WHERE utm_term LIKE '%\%%';
UPDATE public.purchase_tracking SET utm_campaign = public.url_decode(utm_campaign) WHERE utm_campaign LIKE '%\%%';
UPDATE public.purchase_tracking SET utm_content = public.url_decode(utm_content) WHERE utm_content LIKE '%\%%';

-- Rebuild get_dashboard_summary_today
...
BEGIN
  ...
  enriched_purchases AS (
    SELECT
      ...
      COALESCE(p.session_id, psm_fallback.session_id) AS effective_session_id
    FROM public.purchase_tracking p
    CROSS JOIN tz
    LEFT JOIN LATERAL (
      SELECT psm.session_id
      FROM public.phone_session_map psm
      JOIN public.session_attribution sa2 ON sa2.session_id = psm.session_id
      WHERE p.session_id IS NULL AND psm.created_at >= tz.day_start AND sa2.created_at >= tz.day_start
  ...
  sales_breakdown AS (
    SELECT
      ...
      COUNT(*) FILTER (WHERE status = 'approved' AND original_session_id IS NULL) AS orphan_sales
...
-- Rebuild get_campaign_stats_today with URL-decode
...
checkout_agg AS (
  SELECT COALESCE(sc.campaign_label, 'Direto') AS campaign_label, COUNT(DISTINCT fe.session_id) AS ck_count
  FROM public.funnel_events fe LEFT JOIN sa_campaigns sc ON sc.session_id = fe.session_id CROSS JOIN tz
  WHERE fe.created_at >= tz.day_start AND fe.event_name IN ('checkout_click', 'capi_ic_sent') GROUP BY 1
),
sale_details AS (
...
SELECT al.campaign_label AS campaign,
  ...
  COALESCE(sa.sale_count, 0) AS sales, COALESCE(sa.total_revenue, 0) AS revenue,
  COALESCE(sa.refund_count, 0) AS refunds,
  CASE WHEN COALESCE(la.lead_count, 0) > 0
    THEN ROUND((COALESCE(sa.sale_count, 0)::numeric / la.lead_count::numeric) * 100, 1) ELSE 0 END AS conv_rate
FROM all_labels al
